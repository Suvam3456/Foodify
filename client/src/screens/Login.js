...
function Login() {
  ...
  let navigate = useNavigate(); // Home page pe redirect ho jaye submit hote hi
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch("http://localhost:5000/api/loginuser", {
      ...
      // body ko send karenge backend ko json me, but stringify karke bhejenge
    });
    ...
    if (json.success) {
      ...
      navigate("/"); // Home page pe redirect ho jaye submit hote hi
    }
  }
     ...
     <div>
      ...
      <div className='container'>
        <form className='w-50 m-auto mt-5 border bg-dark border-success rounded' onSubmit={handleSubmit}>
          ...
          <button type="submit" className="m-3 btn btn-success" style={{color:"white"}}>Submit</button>
